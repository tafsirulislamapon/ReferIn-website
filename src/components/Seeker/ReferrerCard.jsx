...
export default function ReferrerCard({ 
  ...
  return (
    <>
      <div 
        className={`bg-[#2563EB]/20 backdrop-blur-md rounded-lg p-2.5 sm:p-3 md:p-4 text-white border border-white/30 w-[180px] xs:w-[200px] sm:w-[240px] md:w-[280px] lg:w-[300px] shadow-lg transform transition-transform ${className}`}
      >
        <div className="flex items-start gap-2 sm:gap-3">
          <div className="relative">
            <Image
              ...
              width={28}
              height={28}
              className={`rounded-full object-cover ring-1 ring-white/20 w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 ${!hasPaid ? 'blur-[2px] opacity-70' : ''}`}
            />
            {!hasPaid && (
          ...
          <div className="flex-1 min-w-0">
            <h3 className={`font-clash font-semibold text-[13px] sm:text-sm md:text-base leading-tight mb-0.5 truncate ${!hasPaid ? 'blur-[2px] opacity-70' : ''}`}>
              {name || position}
            </h3>
            <div className="flex items-center gap-1 mb-0.5">
              ...
              <span className="text-[11px] sm:text-xs md:text-sm text-gray-200 truncate">{company}</span>
            </div>
            <div className="text-[11px] sm:text-xs md:text-sm text-gray-300/90 truncate">{location}</div>
          </div>
