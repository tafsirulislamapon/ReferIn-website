...
export default function SuccessPage({ onComplete, referrerCount = 0 }) {
  // Add prop validation
  const count = typeof referrerCount === 'number' ? referrerCount : 0;
      ...
      <div className="relative w-full max-w-[800px] mx-4">
        ...
        >
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-clash font-bold text-white mb-4">
            Great! We found <span className="text-[#4ADE80]">{String(count).padStart(2, '0')}</span> employees in
            <br />total that are open to referring you
          </h1>
        </div>
      </div>
    </div>
